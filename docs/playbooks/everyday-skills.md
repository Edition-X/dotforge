...
**Repo:** `~/Projects/macbook-pro`. Reads `~/Projects/arcane`, `~/Projects/sunrise_ansible_inventory`, `~/Projects/sunrise-cell-deploy-ansible` for facts.
...
6. Conventional Commits with scope `skills` or `ai`. Arcane memory after each commit via `/Users/dkelly/.local/bin/arcane save --project macbook-pro --source claude-code ...`.
