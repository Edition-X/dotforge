...
| Vault | `host_vars/localhost/vault.yml`, edited with `ansible-vault edit` from the repo root after `source venv/bin/activate; unset ANSIBLE_VAULT_PASSWORD_FILE`. Existing keys: `linear_api_key`, `todoist_api_key`, `vault_vault_pass`. |
